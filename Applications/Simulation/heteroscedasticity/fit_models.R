...
g_x <- function(x) x / 10 + 2
  
# Define error function and inverse error function
error <- function(x) rnorm(length(x), 0, abs(x) / 10)
qerror <- function(p, x) qnorm(p, 0, abs(x) / 10)
...
xtable(
  mse,
  caption = "Error cuadrático promedio (datos con heterocedasticidad)",
  align = c("c","c","c"),
  label = "mse_heteroscedasticity"
)
xtable(
  corr,
  caption = "Correlación al cuadrado (datos con heterocedasticidad)",
  align = c("c","c","c"),
  label = "corr_heteroscedasticity"
)
xtable(
  100 * withn,
  caption = "Porcentaje de valores reales dentro del intervalo de confianza (datos con heterocedasticidad)",
  align = c("c","c","c"),
  digits = c(2,0,0),
