...
autobi <- AutoBi %>% 
  select(CLMAGE, LOSS) %>% 
  na.omit %>% 
  filter(LOSS > 8 & CLMAGE > 21) 

ggplot(data = autobi, aes(CLMAGE)) +
...
ohlsson <- dataOhlsson %>% 
  rename(owners_age = agarald , vehicle_age = fordald, cost = skadkost) %>%
  select(owners_age, vehicle_age, cost) %>% 
  filter(cost > 30000 & vehicle_age < 10) 
  
ggplot(data = ohlsson, aes(owners_age)) +
  geom_point(aes(y = cost))

ggplot(data = ohlsson, aes(vehicle_age)) +
  geom_point(aes(y = cost))
...
setwd(paste0(local_path,"Thesis/Applications/Insurance"))
...
g_x <- function(x) return((1/40) * x^2 - (1/20) * x - 2)
...
sample_data <- data_frame(x = x, y = g_x(x) + error(m))
...
# Fit models
GPDP_25 <- GPDPQuantReg(y ~ x, sample_data, p = 0.25, M = zero_function, d_DP = 2, d_lambda = 2, burn = 5e3, mcit = 1e4, thin = 5)
GPDP_50 <- GPDPQuantReg(y ~ x, sample_data, p = 0.50, M = zero_function, d_DP = 2, d_lambda = 2, burn = 5e3, mcit = 1e4, thin = 5)
GPDP_95 <- GPDPQuantReg(y ~ x, sample_data, p = 0.95, M = zero_function, d_DP = 2, d_lambda = 2, burn = 5e3, mcit = 1e4, thin = 5)
